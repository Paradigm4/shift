...
LIBS=-shared -Wl,-soname,libshift.so -L. -L"$(SCIDB_THIRDPARTY_PREFIX)/3rdparty/boost/lib" -L"$(SCIDB)/lib" -Wl,-rpath,$(SCIDB)/lib:$(RPATH) -lm
...
ifneq ("$(wildcard /usr/bin/g++-4.9)","")
  ...
  SRCS+= LogicalShift.cpp PhysicalShift.cpp
else
...
all:
	...
	$(CXX) $(CFLAGS) $(INC) -o libshift.so $(SRCS) $(LIBS)
	@echo "Now copy *.so to your SciDB lib/scidb/plugins directory and run"
	@echo "iquery -aq \"load_library('shift')\" # to load the plugin."
	@echo
